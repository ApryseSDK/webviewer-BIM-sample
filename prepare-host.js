...
exec('cd ../WV3D-MVP && rm -rf dist && npm run build && npm pack && mv pdftron-webviewer-bim-client*.tgz ../startbim/1.tgz && cd ../startbim && rm -rf package && tar -xzvf 1.tgz && rm -rf ./node_modules/@pdftron/webviewer-bim-client && npm i', (err, stdout, stderr) => {
  if (err) {
    console.error(err);
    return;
  }
